...
export function replaceTaskInDocument(source, taskId, newContent) {
    ...
    _extractTask(yamlDoc, taskId, (oldValue) => {
        replaceCommentInTask(oldValue, newItem)
...
export function extractAllTypes(source, validTypes = []) {
    return this.extractFieldFromMaps(source, "type", undefined, value => validTypes.some(t => t === value));
}

export function getTaskType(source, position, validTypes) {
...
export function insertTask(source, taskId, newTask, insertPosition) {
    const yamlDoc = yaml.parseDocument(source);
...
export function insertTrigger(source, triggerTask) {
    ...
    return cleanMetadata(yamlDoc.toString(TOSTRING_OPTIONS));
}

export function insertError(source, errorTask) {
    const yamlDoc = yaml.parseDocument(source);
...
export function insertFinally(source, finallyTask) {
    const yamlDoc = yaml.parseDocument(source);
...
export function insertErrorInFlowable(source, errorTask, flowableTask) {
    const yamlDoc = yaml.parseDocument(source);
    const newErrorNode = yamlDoc.createNode(yaml.parseDocument(errorTask));
    let added = false;
    yaml.visit(yamlDoc, {
...
export function deleteTask(source, taskId, section) {
...
export function checkTaskAlreadyExist(source, taskYaml) {
    const yamlDoc = yaml.parseDocument(source);
    const parsedTask = parse(taskYaml);
    let taskExist = false;
    yaml.visit(yamlDoc, {
        Pair(_, pair) {
...
export function isParentChildrenRelation(source, task1, task2) {
    return isChildrenOf(source, task2, task1) || isChildrenOf(source, task1, task2);
}

export function isChildrenOf(source, parentTask, childTask) {
    const yamlDoc = yaml.parseDocument(extractTask(source, parentTask));
...
export function getChildrenTasks(source, taskId) {
    const yamlDoc = yaml.parseDocument(extractTask(source, taskId));
    const children = [];
    yaml.visit(yamlDoc, {
        Map(_, map) {
            if (map.get("id") !== taskId) {
...
export function getParentTask(source, taskId) {
    const yamlDoc = yaml.parseDocument(source);
    let parentTask = null;
    yaml.visit(yamlDoc, {
        Map(_, map) {
...
export function isTaskError(source, taskId) {
    const yamlDoc = yaml.parseDocument(source);
...
export function isTrigger(source, taskId) {
    const yamlDoc = yaml.parseDocument(source);
    let isTrigger = false;
    yaml.visit(yamlDoc, {
        Pair(_, pair) {
...
export function replaceIdAndNamespace(source, id, namespace) {
    return source.replace(/^(id\s*:\s*(["']?))\S*/m, "$1"+id+"$2").replace(/^(namespace\s*:\s*(["']?))\S*/m, "$1"+namespace+"$2")
}

export function updateMetadata(source, metadata) {
...
export function cleanMetadata(source) {
...
export function flowHaveTasks(source) {
    const yamlDoc = yaml.parseDocument(source);
...
export function deleteMetadata(source, metadata) {
    const yamlDoc = yaml.parseDocument(source);
...
export function getChartAtPosition(source, position) {
    const yamlDoc = yaml.parseDocument(source);
    const lineCounter = new LineCounter();
    yaml.parseDocument(source, {lineCounter});
    const cursorIndex = lineCounter.lineStarts[position.lineNumber - 1] + position.column;
...
export function getAllCharts(source) {
    const yamlDoc = yaml.parseDocument(source);
    const charts = [];
